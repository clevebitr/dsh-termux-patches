...
`apply.sh` 会把改动直接写进 `node_modules`，所以**升级 dsh 后需要重放**。脚本对每个补丁都做了锚点/幂等检查：找不到锚点会报错退出交人工核对，已打过则跳过。最后两步还会做行为断言——硬链接降级覆盖完好（补丁 8）、ripgrep 真的能启动并搜索（补丁 9）——产物被上游还原时直接非零退出中止。
