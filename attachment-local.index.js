...
/** Android/SELinux forbids hard links outright (EACCES even within one's own directory); callers fall back to rename/exclusive copy. */
	...
	try {
		...
		try {
			...
			if (isHardlinkUnsupported(error)) {
				/* Android/SELinux forbids hard links: publish an exclusive copy (source is an immutable content-addressed object and must stay in place). */
				try {
...
async function publishStagedObject(root, target, staged) {
	...
	try {
		...
		let consumedStaged = false;
		try {
			await link(staged.path, target);
		} catch (error) {
			if (isHardlinkUnsupported(error)) {
				/* Android/SELinux forbids hard links: rename publishes atomically and consumes the staged file. */
		...
		if (!consumedStaged) await unlink(staged.path);
		await chmod(target, 256);
		const stop = resolve(root);
		for (let level = parent; level !== stop; level = dirname(level)) {
			await syncDirectory(level);
