...
echo "[1/7] node-pty $PTY_VER: android-arm64/pty.node"
...
echo "[2/7] sharp $SHARP_VER: 安装 @img/sharp-wasm32@$SHARP_VER"
...
echo "[5/7] dsh wrapper → \$PREFIX/bin/dsh"
...
echo "[6/7] client-connection cookie SameSite=Strict → Lax"
...
echo "[7/7] fs-local 硬链接降级(write 工具建新文件)"
