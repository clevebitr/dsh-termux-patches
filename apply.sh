...
echo "[1/10] node-pty $PTY_VER: android-arm64/pty.node"
...
echo "[2/10] sharp $SHARP_VER: 安装 @img/sharp-wasm32@$SHARP_VER"
...
echo "[5/10] dsh wrapper → \$PREFIX/bin/dsh"
...
echo "[6/10] client-connection cookie SameSite=Strict → Lax"
...
echo "[7/10] fs-local 硬链接降级(write 工具建新文件)"
...
echo "[8/10] 会话发布降级(worker.cjs)+ 硬链接降级锚点断言"
...
echo "[9/10] ripgrep 降级到原生 rg + 可执行性断言"
node "$PATCH_DIR/patch-vscode-ripgrep.mjs" "$NM"
node "$PATCH_DIR/check-ripgrep.mjs" "$NM"

# 10. 附件落盘前的祖先目录 fsync 遍历。
#     app 的 home 是 /data/data/com.termux/files/home,而 /data/data 权限 771(可穿越不可读),
#     遍历走到那一级 open(dir, O_RDONLY) 必抛 EACCES → read_image 与聊天贴图整体失败。
#     遍历在系统属主的祖先处收手即可(那些目录安装时就已持久)。
echo "[10/10] 附件持久化遍历降级 + 行为断言"
node "$PATCH_DIR/patch-attachment-durable-walk.mjs" "$NM"
node "$PATCH_DIR/check-attachment-durable.mjs" "$NM"
